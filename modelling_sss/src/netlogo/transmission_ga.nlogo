;; define variables
globals [nTraits traitRange replacementRate moveDistance colorRange outFile] ;; number of traits an agent can possess, number of values that a trait can have,
  ...
  ;; agent colors, name and path of output csv file
...
to create_agents
  ...
  create-agents nAgents [ ;; create a number of agents determined by user-selectable variable nAgents (slider in GUI)
    ...
    set replacementRate 0.05 ;; probability that an agent will reproduce (and one will die) each time step
...
to go
  ...
  ask agents [
    ...
    if transmissionType = "vertical" [vertical]
    if transmissionType = "encounter" [encounter]
    if transmissionType = "prestige" [prestige]
    if transmissionType = "conformist" [conformist]
  ]
  tick
  if saveOutput and ticks mod outputInterval = 0 [output]
  if timeSteps > 0 and ticks = timeSteps [stop]
  
end

to vertical
  if random-float 1 <= replacementRate [
    hatch 1 [
      ;; innovation or copying error
      if random-float 1 <= innovationRate  [
        let place random 3
        set traits replace-item place traits (random 5)
        
        ;; reset agent color to indicate new cultural knowledge
        let r (item 0 traits + 1) * 31
        let b (item 1 traits + 1) * 31
        let g (item 2 traits + 1) * 31
        set color rgb r b g
        ]
      
      ;; kill off one agent to keep population stable
      ask one-of agents [die]
      ]    
    ]
end
...
to encounter
  ...
  let place random 3 ;; select trait from trait list to replace
    
  ;; simple copying of a trait value from an agent in the interaction radius
  let partner one-of agents-here
  if partner != nobody [ ;; pick another agent in same patch
    set information [item place traits] of partner ;; copy information from partner
    set traits replace-item place traits information ;; learn new information
    ]
  
  ;; innovation or copying error
  if random-float 1 <= innovationRate [ 
    set information random 5
    set traits replace-item place traits information
    ]
  
  ;; set agent color to indicate cultural knowledge
  let r (item 0 traits + 1) * colorRange
  let b (item 1 traits + 1) * colorRange
  let g (item 2 traits + 1) * colorRange
  set color rgb r b g
...
to prestige
  ...
  ;; probability of copying a trait from a neighbor agent 
  ;;determined by that agent's prestige trait value (trait0)
  let partner one-of agents in-radius interactionRadius
  let prestigePlace 0
  if partner != nobody [
    let prestigeValue [item prestigePlace traits] of partner ;; evaluate prestige of partner
    if random-float 1 <= (prestigeValue + 1) / traitRange [ ;; prestige of partner determins proability that value will be copied
      set information [item place traits] of partner ;; copy information from partner
      set traits replace-item place traits information ;; learn new information
    ]
  ]
  ...
  ;; innovation or copying error
  if random-float 1 <= innovationRate [ ;; innovation or copying error
    set information random 5
    set traits replace-item place traits information
    ]
...
to conformist
  ...
  ;; replace a trait with the most common value among agents in neighborhood
  let partners agents in-radius interactionRadius
  if partners != nobody [
    let popularity []
    foreach n-values traitRange [?]  [
      set popularity lput (count partners with [item place traits = ?]) popularity
      ]
    set information position (max popularity) popularity ;; copy most popular information
    set traits replace-item place traits information ;; learn new information
  ]
...
to move
    rt random 360
    if patch-ahead moveDistance = nobody [ rt random 360 lt random 360]
    fd moveDistance
end

to output
  ask agents [
    file-type who file-type "," file-type ticks file-type "," file-print traits
  ]
